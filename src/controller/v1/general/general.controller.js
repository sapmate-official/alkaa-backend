...
const verifyPasswordAndSendOtp = async (req, res) => {
    try {
        ...
        await sendLoginOTPEmail(user.email,`${user.firstName} ${user.lastName}`,otpCode,user.organization.name,{    ip: req.ip,    userAgent: req.get('User-Agent'),    timestamp: new Date()    });
